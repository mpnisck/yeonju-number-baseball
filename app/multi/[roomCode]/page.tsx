...
import { useMultiplayerGame, GameRoom } from "@/app/hooks/useMultiplayerGame";
...
function LoadingDots({
  ...
  return (
    <div className="flex items-center gap-2.5">
      <div className={`w-2.5 h-2.5 rounded-full ${color} animate-bounce-dot`} />
      <div
        className={`w-2.5 h-2.5 rounded-full ${color} animate-bounce-dot`}
...
function PlayerReadyStatus({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={`text-xs font-bold ${ready ? "text-[var(--success)]" : "text-[var(--text-muted)]"}`}
      >
        {ready ? "준비 완료" : "준비 중..."}
      </span>
    </div>
...
function SpectatorWaitingView({ roomCode }: { roomCode: string }) {
  return (
      ...
      <div className="w-full max-w-sm flex flex-col gap-8 animate-fade-in">
        ...
        <div className="flex flex-col items-center gap-6 py-10">
          ...
          <div className="flex items-center gap-6">
            <PlayerReadyStatus player={1} ready={room.player1_ready} />
            <PlayerReadyStatus player={2} ready={room.player2_ready} />
          </div>

          <LoadingDots color="bg-[var(--spectator)]" />
        </div>
      </div>
    </main>
  );
...
function PlayerWaitingView({
  ...
  return (
    <main className="min-h-dvh flex flex-col items-center px-6 py-10 sm:py-16">
      <div className="w-full max-w-sm flex flex-col gap-8 animate-fade-in">
        <GameHeader title="온라인 대전" subtitle={`방 코드: ${roomCode}`} />
        ...
        <div className="flex items-center justify-center gap-2.5 py-3.5 rounded-xl border border-[var(--border)] text-[var(--text-primary)]">
          ...
          <span className="text-sm font-bold">Player {playerNumber}</span>
...
function PlayerSetupReadyView({ roomCode }: { roomCode: string }) {
  return (
    <main className="min-h-dvh flex flex-col items-center px-6 py-10 sm:py-16">
      <div className="w-full max-w-sm flex flex-col gap-8 animate-fade-in">
        <GameHeader title="온라인 대전" subtitle={`방 코드: ${roomCode}`} />
...
export default function MultiGamePage() {
  ...
  if (isSpectator) {
    if (room.status === "waiting") {
      return <SpectatorWaitingView roomCode={roomCode} />;
    }
    if (room.status === "setup") {
      return <SpectatorSetupView roomCode={roomCode} room={room} />;
    }
  }
  ...
  if (!isSpectator) {
    if (room.status === "waiting") {
      return (
        <PlayerWaitingView roomCode={roomCode} playerNumber={playerNumber!} />
      );
    }

    if (room.status === "setup") {
      if (myReady) {
        return <PlayerSetupReadyView roomCode={roomCode} />;
      }
    ...
    const statusBadge = (() => {
      if (isFinished) {
        return {
          ...
          content: <span className="font-bold text-sm">게임 종료</span>,
        };
      }
      if (isSpectator) {
        return {
      ...
      <main className="min-h-dvh flex flex-col items-center px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="w-full max-w-3xl animate-fade-in">
          <GameHeader
            title="온라인 대전"
            subtitle={`방 코드: ${roomCode}`}
            rightContent={isSpectator ? <SpectatorBadge /> : undefined}
          >
        ...
        {isFinished && room.winner && (
          <VictoryOverlay
            ...
            mySecret={isSpectator ? room.player1_secret : mySecret}
            opponentSecret={isSpectator ? room.player2_secret : opponentSecret}
            secretLabels={
              isSpectator ? { my: "P1 숫자", opponent: "P2 숫자" } : undefined
            }
